...
var Zookeeper = function(options) {
  ...
  config.zkPath = config.zkPath || '/';
  config.noMessageTimeout = config.noMessageTimeout || 10000;
  config.failedMessageTimeout = config.failedMessageTimeout || 5000;
  log = config.logger || new Logger();
};
    ...
    that.decompressMessages(messages, function(error, result) {
      if (messages.length == 0) {
        log.debug('No messages');
          return setTimeout(function() { onConsumerReady(consumer) }, config.noMessageTimeout);
      }
      ...
      var onAck = function(ack) {
        if (ack) {
          ...
          log.warn('Messages failed by client; retrying with delay...');
          setTimeout(function() { onConsume(null, messages); }, config.failedMessageTimeout);
        }
      };

      cb(error, result, onAck, newOffset);
    });
  }
};
...
Zookeeper.prototype.getBrokers = function(cb) {
  ...
  var that = this, brokerIdPath = path.join(config.zkPath,'/brokers/ids');
...
Zookeeper.prototype.getConsumerOffsets = function(topic, group, cb) {
  ...
  var that = this, groupTopicPath = path.join(config.zkPath,'/consumers/' + group + '/offsets/' + topic);
  var zk = new zkcli(zkOpts);
...
Zookeeper.prototype.setConsumerOffsets = function(topic, group, offsets, cb) {
  ...
  var groupTopicPath = path.join(config.zkPath,'/consumers/' + group + '/offsets/' + topic);
  var zk = new zkcli(zkOpts);
...
Zookeeper.prototype.getPartitions = function(topic, broker, cb) {
  ...
  var topicBrokerPath =  path.join(config.zkPath,'/brokers/topics/' + topic + '/' + broker.id);
  var zk = new zkcli(zkOpts);
