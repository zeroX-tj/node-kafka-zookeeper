...
Zookeeper.prototype.initializeConsumerOffsets = function(topic, group, cb) {
  ...
  var onGetBrokers = function(brokers, error) {
    ...
    _.each(brokers, initializeBrokerOffset);
  };
  ...
  var onGetTopicBrokerPartitions = function(topic, broker, partitions, error) {
    log.debug('initializeConsumerOffsets/onGetTopicBrokerPartitions/broker', partitions, error);
    ...
    this.onInitializePartition = _.after(partitions, partitionsInitialized);

    // Initialize each broker/partition with a 0 offzet
  ...
  var initializePartition = function(partition, broker) {
    ...
    zk.a_exists(offsetPath, false, function(rc, error, stat) {
      if (rc != 0) {
        if (error == 'no node') return zk.a_create(offsetPath, '0', null, this.onInitializePartition);
        return onError('Error retriving offset: ' + error);
      } else {
        if (zk.state != 3) console.trace('Invalid Zk state for query: ' + zk.state);
        zk.a_set(offsetPath, 0, stat.version, this.onInitializePartition);
      }
    });
  };

  var partitionsInitialized = function() {
    log.debug('initializeConsumerOffsets/initializePartition/partitionsInitialized');
    cb();
  };
