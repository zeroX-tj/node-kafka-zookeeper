    ...
    if (error) {
      if (error.message == 'OffsetOutOfRange') {
        ...
        var onInitializeConsumerOffset = function(error) {
          log.debug('consumeTopic/onConsume/onInitializeConsumerOffset/error', error);
...
Zookeeper.prototype.getConsumerOffsets = function(topic, group, cb) {
  ...
  var that = this, result = [], errors = [];
  var groupTopicPath = '/consumers/' + group + '/offsets/' + topic;
  var onOffsetsProcessed;

  // After connected, ensure path exists for group/topic (under which
  // are broker-partition nodes, each containing an offset)
  var onZkConnect = function(error) {
  ...
  // Once we're sure the group/topic path exists, retrieve its children
  ...
  var onGetBrokerPartitions = function(rc, error, brokerPartitions) {
    log.debug('getConsumerOffsets/onGetBrokerPartitions/brokerPartitions', brokerPartitions);
    ...
    that.onOffsetsProcessed = _.after(brokerPartitions.length, function(errors) {
      log.debug('getConsumerOffsets/onOffsetsProcessed');
      zk.close();
      return cb(result, (errors && !_.isEmpty(errors)) ? errors : undefined);
    });

    // If no broker-partition nodes exist, defer to initializeConsumerOffsets to create them
    // and then start over. Otherwise, retrieve each.
    if (brokerPartitions.length == 0) {
      log.debug('getConsumerOffsets/onGetBrokerPartitions/brokerPartitions.length == 0');
      return that.initializeConsumerOffsets(topic, group, function(error) {
        if (error) return cb(null, error);
        that.getConsumerOffsets(topic, group, cb);
      });
    } else {
      _.each(brokerPartitions, getBrokerPartition);
    }
  };

  var getBrokerPartition = function(brokerPartitionString) {
    log.debug('getConsumerOffsets/getBrokerPartition/brokerPartitionString', brokerPartitionString);
  ...
  var onGetBrokerPartition = function(rc, error, data, brokerPartitionString) {
    ...
    that.onOffsetsProcessed(errors);
  };
...
Zookeeper.prototype.setConsumerOffsets = function(topic, group, offsets, cb) {
  ...
  var getOffset = function(offset) {
    ...
    zk.a_exists(offsetPath, false, function(rc, error, stat) {
      if (rc != 0) {
        if (error == 'no node') return zk.a_create(offsetPath, offset, null, onOffsetProcessed);
...
Zookeeper.prototype.initializeConsumerOffsets = function(topic, group, cb) {
  ...
  var zkConnect = function() {
    log.debug('initializeConsumerOffsets/zkConnect/zk\n', JSON.stringify(zk, null, 2));
    zk.connect(onZkConnect);
  };

  // Once connected, retrieve broker list from Zookeeper
  var onZkConnect = function(error) {
    if (error) {
      log.error('Error connecting to Zookeeper: ' + error);
      return setTimeout(zkConnect, 1000);
    }
  ...
  var onGetBrokers = function(brokers, error) {
    log.debug('initializeConsumerOffsets/onGetBrokers/brokers\n', brokers, error);
    if (error) return onError('Error retrieving brokers: ' + error);

    // Now that we know how many brokers we have, set completion function
    var allBrokersInitialized = _.after(brokers.length, cb);

    // Retrieve partitions for each broker
    _.each(brokers, function(broker) {
      initializeBrokerOffsets(broker, allBrokersInitialized);
    });
  };

  var initializeBrokerOffsets = function(broker, initOffsetsCb) {
    log.debug('initializeConsumerOffsets/initializeBrokerOffset/broker', broker);

    that.getTopicBrokerPartitions(topic, broker, function(topic, broker, partitions, error) {
      log.debug('initializeConsumerOffsets/onGetTopicBrokerPartitions/partitions', partitions, error);
      if (error) return onError('Error retrieving topic/broker partitions: ' + error);

      // Now that we know how many partitions we'll be initializing, set completion function
      log.debug('initializeConsumerOffsets/onGetTopicBrokerPartitions/partitions', partitions);
      that.onPartitionInitialized = _.after(partitions, initOffsetsCb);

      // Initialize each broker/partition with a 0 offset
  ...
  var initializePartition = function(partition, broker) {
    log.debug('initializeConsumerOffsets/initializePartition/brokerPartition', broker.id + '-' + partition);
    var groupTopicPath = '/consumers/' + group + '/offsets/' + topic;
    var offsetPath = groupTopicPath + '/' + broker.id + '-' + partition;
    log.info('Initializing offset to 0: %s', offsetPath);

    if (zk.state != 3) console.trace('Invalid Zk state for query: ' + zk.state);
    zk.a_exists(offsetPath, false, function(rc, error, stat) {
      if (rc != 0) {
        if (error == 'no node') return zk.a_create(offsetPath, '0', null, that.onPartitionInitialized);
        return onError('Error retriving offset: ' + error);
      } else {
        if (zk.state != 3) console.trace('Invalid Zk state for query: ' + zk.state);
        zk.a_set(offsetPath, 0, stat.version, that.onPartitionInitialized);
      }
    });
  };
