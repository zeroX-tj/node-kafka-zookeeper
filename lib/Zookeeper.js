  ...
  async.waterfall([
      ...
      function connectConsumers(consumers, callback) {
        ...
        async.each(consumers, function(consumer, connectCallback) {
          ...
          // TODO: Review onConsumerReady call
          consumer.connect(function(error) { onConsumerReady(consumer, error); });
        });
    ...
    if (error) {
      if (error.message == 'OffsetOutOfRange') {
        ...
        that.initializeConsumerOffset(consumer, group, function(error) {
          log.debug('consumeTopic/onConsume/initializeConsumerOffset/error', error);
        ...
        console.log(consumer.partition);
      ...
      function initializeOffsetsConditional(brokers, offsets, callback) {
        ...
        if (_.isEmpty(offsets)) {
          ...
          return that.initializeConsumerOffsets(topic, group, function(error) {
            ...
            // TODO: DCROUSE HERE
            log.debug('getConsumers/initializeOffsetsConditional/getConsumers');
            return that.getConsumers(topic, group, function(error, result) { callback(error, result); });
          });
        } else {
          return callback(null, brokers, offsets);
        }
      },
          ...
          zk.a_exists(offsetPath, false, function(rc, error, stat) {
            if (rc != 0) {
              if (error == 'no node') {
                ...
                return zk.a_create(offsetPath, '0', null, function(rc, error) {
                  return partitionCb((rc != 0) ? error : null);
                });
              } else {
                log.debug('initializeConsumerOffsets/initializePartitions/unknownError');
                return partitionCb(error);
              }
            } else {
  ...
  async.waterfall([
      ...
      function getPartitions(callback) {
        zk.a_get(topicBrokerPath, false, function(rc, error, stat, data) {
          ...
          var result = _.repeat(',', partitionCount).split(',');
